	...
	<div class="card mb-4">
		...
		<div class="card-body row g-3">
			...
			<div class="col-md-6">
				...
				<input type="number" name="employees" class="form-control @error('employees') is-invalid @enderror" value="{{ old('employees') }}">
				@error('employees')
					<div class="invalid-feedback">{{ $message }}</div>
				@enderror
			</div>
	...
	<div class="card mb-4">
		...
		<div class="card-body row g-3">
			@foreach ($dateFields as $field => $label)
				<div class="col-md-4">
					<label class="form-label">{{ $label }}</label>
					<input type="datetime-local" name="{{ $field }}" class="form-control @error($field) is-invalid @enderror" value="{{ old($field) }}">
					@error($field)
						<div class="invalid-feedback">{{ $message }}</div>
					@enderror
				</div>
			...
			@foreach ($textareaFields as $field => $label)
				<div class="col-12">
					<label class="form-label">{{ $label }}</label>
					<textarea name="{{ $field }}" rows="4" class="form-control @error($field) is-invalid @enderror">{{ old($field) }}</textarea>
					@error($field)
						<div class="invalid-feedback">{{ $message }}</div>
					@enderror
				</div>
